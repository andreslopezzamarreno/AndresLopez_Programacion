...
public class Ejercicio1 {
    ...
    public static void main(String[] args) {
        ...
        // int[] nums = new int[0];    no crear array con posiciones 0, crearlo nulo.
        int[] nums = null ;
        boolean creacion = false;
        int opcion;
        //int tamaño = -1;    no utilizar ñ
        int tamanio = -1;
        int posInicial;

        do {
            System.out.print("1.Introducir tamaño del Array \n2.Rellenar Array \n3.Sacar posiciones donde el valor sea mayor que 10 y par " +
            ...
            switch (opcion) {
                ...
                case 3:
                    System.out.println("Sacar posiciones donde el valor sea mayor que 10 y par ");
                    if (creacion) {
                        for (int i = 0; i < nums.length; i++) {
                ...
                case 4:
                    if (creacion) {
                        ...
                        posInicial = nums[0];
                        for (int i = 0; i < nums.length; i++) {
                            //if (nums[i] != nums[nums.length - 1]) {      fallo, evaluar posicion no datos
                            if (i != nums.length - 1) {
                                nums[i] = nums[i + 1];
                            } else {
                                nums[i] = posInicial;
